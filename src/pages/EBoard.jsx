...
export default function EBoard() {
  return (
    <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      {/* Page header */}
      <div className="mb-14">
        <h1 className="text-4xl font-bold text-ieee-navy mb-4">Executive Board</h1>
      ...
      <div className="flex flex-wrap gap-3 mb-16">
        {sectors.map(({ id, title }) => (
          <a
            key={id}
            href={`#${id}`}
            className="text-sm px-4 py-1.5 rounded-full border border-ieee-border text-ieee-dark hover:border-ieee-teal hover:text-ieee-teal transition-all duration-200"
          >
            {title}
          </a>
        ))}
